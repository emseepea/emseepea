...
This project has 94 decisions: 54 current and 40 historical.
...
Chosen option: **"Checked multi-content resource reads"**, because Model Context Protocol (MCP) allows one resource read to return several content items. Em See Pea will still treat the originally requested resource as the only authorized read.
...
- Results invalid under the software development kit (SDK) schema and oversized results retain the generic safe resource error.
