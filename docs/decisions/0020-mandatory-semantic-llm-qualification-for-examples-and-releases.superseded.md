...
- Avoid overstating results beyond the provider, model, cases, and commit tested.
...
Promptfoo is mandatory for every example. The authoritative CI provider and
judge are GitHub Copilot CLI pinned to `claude-sonnet-4.6`; GitHub Models is not
used. Local Claude execution may implement the same cases for advisory feedback
but cannot satisfy the release gate.

Each semantic case runs three fresh, uncached agent trials and three independent
judge verdicts. All three trials must pass the deterministic critical facts, the
meaning rubric, and the required path evidence. A correct-looking answer without
the required MCP evidence fails. Tool cases must record the named MCP server and
tool calls. Resource and prompt cases must record the exact official-client
read or get operation and are described as semantic consumption, not autonomous
resource or prompt selection.

There are six graded provider outputs per case: three agent answers and three
judge verdicts. Internal tool-loop inference is separate and bounded to one
named MCP server, at most three MCP tool calls per agent trial, a Copilot
response-credit cap, a provider timeout, and a job timeout. The judge receives
no MCP access.
...
Promptfoo, Copilot CLI, and their exact dependency closure are development-only
and pinned in the root lockfile. The eval process disables cache, telemetry,
update checks, sharing, remote generation, ambient `.env` loading, persistent
configuration, custom instructions, plugins, subagents, shell access, filesystem
write access, arbitrary URLs, and unrelated MCP servers. The Copilot credential
is passed only to the Copilot child through an allowlisted environment.
...
dependency versions, effective provider and model, configuration digest, case and
trial results, path evidence, internal turn and tool counts, rubric, transport
retries, and errors. Release claims name only that evidence, provider, model,
case set, and commit; they never claim generic LLM understanding.
...
- Failures expose whether facts, meaning, or MCP use caused the qualification gap.
...
- Resource and prompt cases prove consumption through an official client rather
  than claiming an LLM selected those capabilities autonomously.
...
- Every example has at least one Promptfoo case with deterministic critical facts,
  a semantic rubric, and explicit MCP path evidence.
- Every case runs three uncached agent trials and three judge verdicts, and all
...
- The release job depends on a passing semantic evaluation of the exact publishing
...
- Bad: Cannot detect an LLM drawing the wrong business conclusion from correct data.
