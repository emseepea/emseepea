...
const secondRepairMerge = "1".repeat(40);
...
function fixture({ repair = false, secondRepair = false, runs = [successful()], origin = sourceSha, diff, website = "0.1.0", expectedRepairBase = secondRepair ? repairMerge : originalMerge, expectedOriginalMerge = originalMerge, includeOriginalMerge = true, defaultLookup = false, unrelated = false } = {}) {
  const head = secondRepair ? secondRepairMerge : repair ? repairMerge : originalMerge;
  const outputs = {
    "rev-parse HEAD": head,
    "rev-parse HEAD^2": repair ? repairHead : releaseSha,
    "rev-parse HEAD^1": secondRepair ? repairMerge : originalMerge,
    "rev-parse HEAD^1^2": secondRepair ? repairHead : releaseSha,
    "rev-parse HEAD^1^1": originalMerge,
    "rev-parse HEAD^1^1^2": releaseSha,
  ...
  const options = {
    env: { GITHUB_SHA: head, GITHUB_REPOSITORY: "emseepea/emseepea", EMSEEPEA_REPAIR_BASE_SHA: expectedRepairBase, ...(secondRepair && includeOriginalMerge ? { EMSEEPEA_ORIGINAL_PUBLISH_SHA: expectedOriginalMerge } : {}) },
...
test("a second promotion-only repair binds through both failed publish merges", async () => {
  assert.deepEqual(await bindPromotionRelease(fixture({ repair: true, secondRepair: true })), { releaseSha, releaseRunId: "123" });
  await assert.rejects(
    () => bindPromotionRelease(fixture({ repair: true, secondRepair: true, includeOriginalMerge: false })),
    /successful Release run/,
  );
  await assert.rejects(
    () => bindPromotionRelease(fixture({ repair: true, secondRepair: true, expectedOriginalMerge: "f".repeat(40) })),
    /original publish merge/,
  );
});
...
test("repair refuses a changed first parent, disallowed source edit, or website release", async () => {
  await assert.rejects(() => bindPromotionRelease(fixture({ repair: true, expectedRepairBase: "f".repeat(40) })), /expected publish merge/);
