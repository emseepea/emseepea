...
Install the optional stylesheet package, then import its one CSS file from your
application entry point:

```sh title="Install the optional stylesheet"
npm install @emseepea/tailwind
```

```ts title="Import the optional stylesheet"
import "@emseepea/tailwind/styles.css";
```

The same import styles native `renderResultView`, React `ResultCard`, and
...
Your application still owns domain mapping, wording, business policy, actions,
effect authorization, the page shell, theme application, branding, and style
overrides. Removing the import changes presentation only.
...
packed packages. The cross-renderer contract tests
