---
title: Test whether AI understands your results
description: Write JavaScript tests for the meaning of MCP results, separate from ordinary tests.
---
...
This guide describes `@emseepea/testing` 0.1.0. The examples use
`@emseepea/server` 0.0.3. Both are pre-alpha packages.

## Use it with your existing server

`@emseepea/testing` does not depend on the Em See Pea server package. You can
keep your current server and try one meaning test before considering a
framework migration.
...
Start with one result where a plausible misunderstanding would matter. For a
loan calculator, distinguish a lender's assessment rate from the rate used to
estimate actual repayments. For accounting, distinguish an overdraft from cash
available to spend, or a payment received from one matched to an invoice.

These checks supplement your existing tests. They do not replace tool-selection
tests, real ChatGPT or Codex journeys, or checks of permissions and business rules.
