...
test("two server processes share one atomic report store", async (t) => {
  ...
  assert.deepEqual(fromFirst.roastCounts, { light: 1, medium: 2, dark: 1 });
  assert.equal(fromFirst.totalBeans, 4);
  ...
  assert.doesNotMatch(JSON.stringify(unavailable.body), /sqlite|database|closed|provider/i);
...
test("an unavailable SQLite provider does not stop independent server features", async (t) => {
  ...
  assert.ok(listed.tools.some(({ name }) => name === "create-shared-bean-report"));
  assert.deepEqual(
  ...
  assert.equal(unavailable.body.result.content[0].text, "Tool execution failed");
  assert.doesNotMatch(JSON.stringify(unavailable.body), /sqlite|database|missing|provider/i);
...
async function rawCreateReport(url, requestId) {
  const response = await fetch(url, {
    ...
    headers: {
      ...
      "Mcp-Name": "create-shared-bean-report",
    },
    body: JSON.stringify({
      jsonrpc: "2.0",
