...
Use this template when two local server processes must share one SQLite-backed
report store and repeated request IDs must resolve to one stored report. Choose
the [tool server](../tool-server/README.md) when one process and in-memory data
are enough. This template does not coordinate across computers. [Compare all eight templates](https://emseepea.github.io/emseepea/examples/).
...
The same request ID creates one stored pea harvest report. A retry through either
...
- `create-shared-harvest-report` creates or replays a stored report for a request
